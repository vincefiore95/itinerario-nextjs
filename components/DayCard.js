...
export default function DayCard({ day, onDelete }) {
  ...
  return (
    <div className="card" role="group" aria-label={`Itinerario ${day.title}`}>
      ...
      <style jsx>{`
        .card{display:grid;gap:.6rem;padding:18px;border-radius:16px;background:var(--card);border:1px solid var(--border);box-shadow:0 1px 2px rgba(0,0,0,.05);transition:transform .12s, box-shadow .2s, border-color .2s;}
        .card:hover{transform:translateY(-2px);box-shadow:0 10px 24px rgba(0,0,0,.08);border-color:var(--brand);}
        .date{width:fit-content;font-size:.78rem;padding:.28rem .6rem;border-radius:999px;background:#fff7d6;border:1px solid #ffe08a;color:#7a5a00;}
        ...
        .btn{background:var(--brand);color:#111;border:1px solid var(--brand);padding:8px 12px;border-radius:10px;font-weight:600;cursor:pointer;text-decoration:none}
